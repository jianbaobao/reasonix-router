#!/bin/bash
# ============================================================
#  Reasonix Router — 全自动 Ubuntu 软路由 ISO 生成器
#
#  用法:
#    sudo ./rebuild-iso.sh                     # 自动下载 Ubuntu + 构建
#    sudo ./rebuild-iso.sh ./ubuntu-24.04.iso  # 使用本地 ISO
#    sudo ./rebuild-iso.sh --no-download       # 不下载，用本地已有
#
#  输出: dist/ubuntu-reasonix-router-24.04.iso
# ============================================================
set -e
...
UBUNTU_URL="https://releases.ubuntu.com/${UBUNTU_VERSION}/${UBUNTU_ISO_NAME}"
OUTPUT_NAME="ubuntu-reasonix-router-${UBUNTU_VERSION}.iso"
...
check_deps() {
    local missing=()
    for d in xorriso wget sed; do
        command -v "$d" &>/dev/null || missing+=("$d")
    done
    if [ ${#missing[@]} -gt 0 ]; then
        echo "安装依赖: sudo apt install xorriso wget -y"
        sudo apt install xorriso wget -y 2>/dev/null || error "安装失败"
    fi
}

# ─── 准备 autoinstall user-data ─────────────────────────────
generate_userdata() {
    info "生成 autoinstall 配置..."
    mkdir -p "$WORK_DIR/nocloud"

    cat > "$WORK_DIR/nocloud/user-data" << 'CLOUDEOF'
#cloud-config
autoinstall:
  version: 1
  keyboard: { layout: us }
  ...
  identity:
    ...
    username: router
    password: "$6$rounds=4096$BwJjXxZ$Q"
  ssh: { allow-pw: true, authorized-keys: [] }
  packages:
    - dnsmasq
    - lighttpd
    - iptables-persistent
  ...
  late-commands:
    - |
      curtin in-target --target=/target -- bash -c '
        set -e
        echo ">>> 配置网络接口..."
        WAN=$(ip -o link show | grep -v lo | head -1 | awk -F": " "{print \$2}")
        LAN=$(ip -o link show | grep -v lo | tail -1 | awk -F": " "{print \$2}")
        [ -z "$WAN" ] && WAN=ens33; [ -z "$LAN" ] && LAN=ens34
...
network:
  ...
  renderer: networkd
  ethernets:
    $WAN: { dhcp4: true, dhcp4-overrides: { route-metric: 100 } }
    $LAN: { addresses: [192.168.2.1/24], dhcp4: false }
NETEOF
...
LIGHTEOF
        ...
        echo ">>> 完成! 路由地址: http://192.168.2.1"
      '
  shutdown: reboot
CLOUDEOF

    touch "$WORK_DIR/nocloud/meta-data"
    ok "autoinstall 配置已生成"
}

# ─── 生成 Web UI 文件 ──────────────────────────────────────
generate_webui() {
    info "生成 Web UI 文件..."
    local wd="$WORK_DIR/webui"
    mkdir -p "$wd/cgi-bin"

    # index.html
    cat > "$wd/index.html" << 'HTML'
...
.lang-btn{background:0;border:1px solid #2a5a8a;color:#4fc3f7;padding:6px 16px;border-radius:4px;cursor:pointer;font-size:.85em;white-space:nowrap}
...
async function refreshAll(){refreshStatus();refreshNetwork();refreshDHCP();refreshSystem()}
document.addEventListener('DOMContentLoaded',()=>{setTimeout(refreshAll,100)})
APPJS
    ok "app.js 已生成"

    # CGI API 脚本
    cat > "$wd/cgi-bin/api.sh" << 'APISH'
#!/bin/bash
...
# 缓存 POST 数据
POST_DATA=""
[ "$METHOD" = "POST" ] && [ -n "$CONTENT_LENGTH" ] && [ "$CONTENT_LENGTH" -gt 0 ] 2>/dev/null && \
...
case "$RQ" in
  uptime)   u=$(cat /proc/uptime|awk '{print $1}')
            d=$(echo "$u/86400"|bc 2>/dev/null)
            h=$(echo "($u%86400)/3600"|bc 2>/dev/null)
            m=$(echo "($u%3600)/60"|bc 2>/dev/null)
  ...
  interfaces)
    ...
    for d in /sys/class/net/*/; do
        ...
        out="${out}{\"name\":\"$n\",\"ip\":\"$ip\",\"mac\":\"$a\",\"up\":$up},"
    done
    echo "{\"interfaces\":[${out%%,}]}" ;;
  ...
  dhcp-leases)
    ls=""
    [ -f /var/lib/misc/dnsmasq.leases ] && ls=$(cat /var/lib/misc/dnsmasq.leases|while read e m i h r;do
        [ -z "$m" ]&&continue; now=$(date +%s); rem=$((e-now))
        [ "$rem" -lt 0 ]&&continue; [ "$rem" -gt 86400 ]&&rs=">24h"||rs="${rem}s"
...
build_iso() {
    ...
    # 修改 GRUB 启动参数
    find "$WORK_DIR/iso/boot/grub" -name "grub.cfg" 2>/dev/null | while read f; do
        sed -i 's|linux	/casper/vmlinuz|linux	/casper/vmlinuz autoinstall ds=nocloud\\;s=/cdrom/nocloud\\;|g' "$f"
        sed -i 's|linux	/boot/grub/loopback.cfg/||g' "$f" 2>/dev/null || true
    done
    ok "启动参数已注入"

    info "部署 Web UI..."
    mkdir -p "$WORK_DIR/iso/var/www/html"
    cp -r "$WORK_DIR/webui/"* "$WORK_DIR/iso/var/www/html/" 2>/dev/null || true
    ok "Web UI 已集成"

    info "打包 ISO..."
    mkdir -p "$DIST_DIR"
    local out="$DIST_DIR/$OUTPUT_NAME"
    xorriso -as mkisofs -r -V "ReasonixRouter" -J -joliet-long \
    ...
    if [ -f "$out" ]; then
        ...
        echo "║                                         ║"
        echo "║ 使用: 写入U盘 → 启动 → 自动安装 → 路由 ║"
        echo "║ Web:  http://192.168.2.1                 ║"
        echo "║ SSH:  router@192.168.2.1  密码: admin   ║"
        echo "╚══════════════════════════════════════════╝"
    else
        error "ISO 构建失败!"
    fi
...
main() {
    echo ""
    echo "╔══════════════════════════════════════════╗"
    echo "║  Reasonix Router — 全自动 ISO 生成器    ║"
    echo "║  基于 Ubuntu $UBUNTU_VERSION Server       ║"
    echo "╚══════════════════════════════════════════╝"
    echo ""
